...
namespace QLSV_ConnectDB.BLL
{
    public class SinhVienBLL
    {
        ...
        public List<SinhVien> GetDS() => dal.LayTatCa();
        ...
        public string CheckAndSave(SinhVien sv, bool isUpdate)
        {
            ...
            if (!isUpdate)
            {
                // Kiểm tra trùng mã trực tiếp từ Database
                if (dal.LayTatCa().Any(x => x.MaSV.Trim().Equals(sv.MaSV.Trim(), StringComparison.OrdinalIgnoreCase)))
